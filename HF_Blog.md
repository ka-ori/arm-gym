...
Building an RL environment for a task like this has a hidden engineering problem: the training loop (which runs on GPU) and the verification pipeline (which runs QEMU, the assembler, and LLVM-MCA on CPU) are completely different kinds of work. If you mix them together in the same process, the GPU sits idle while QEMU is running, and you waste expensive compute.

OpenEnv solves this by separating them cleanly. The environment runs as its own server. The training loop talks to it over HTTP or WebSocket. Neither side knows about the other's internals.

Here is what that looks like in practice for ARM-Gym.

The environment exposes a small, standard API defined in `openenv.yaml`:

```
POST /reset   → gives the trainer a kernel to optimize (C source + baseline assembly + cycle count)
POST /step    → takes the model's assembly, runs the 3-gate verifier, returns the reward
GET  /state   → current episode info (kernel name, difficulty stage, best speedup seen)
WS   /ws      → same operations over WebSocket for lower latency
```

The trainer (TRL's GRPOTrainer) never touches QEMU, the GNU assembler, or LLVM-MCA. It sends a POST request with the model's generated assembly and gets back a reward score. The entire verification stack - syntax check, QEMU correctness runs, LLVM-MCA cycle count, cross-check ratio, 3-sigma bound - happens inside the environment server, invisibly.

**What `reset` returns:**

```json
{
  "variant_id": "vec_add_n16_float32",
  "c_source": "void kernel(float *a, ...) { ... }",
  "baseline_asm": ".text\n.global kernel\n...",
  "baseline_cycles": 412.0,
  "difficulty": 1
}
```

The trainer feeds `c_source` and `baseline_asm` directly into the model prompt. `baseline_cycles` is the number to beat.
...
```json
{
  ...
  "speedup": null,
  "done": true,
  "error_json": "{\"kind\": \"syntax_error\", \"line\": 14, \"message\": \"unknown mnemonic\"}"
}
```

That `error_json` field is key. When the model produces broken assembly, it gets back exactly what went wrong - the error kind, the line number, the assembler message. The next generation prompt can include this, letting the model self-correct. No reward, but structured feedback instead of silence.

**Concurrent sessions.** The environment declares `supports_concurrent_sessions: true` in `openenv.yaml`. This means multiple GPU training workers can hit the same environment server at the same time, each running independent episodes on different kernel variants. The environment handles this safely because each request is stateless at the verification level - QEMU runs are isolated, LLVM-MCA is read-only, and the curriculum state is shared and thread-safe.

**The curriculum lives inside the environment.** The stage-gating logic (advance when 80% of variants at current difficulty pass) runs automatically inside `ARMGymEnv` on every `step` call. The trainer does not need to implement or even know about curriculum progression. It just calls `reset` and `step`. The environment silently graduates the difficulty when the model is ready.

In short: OpenEnv let us write the hard parts (verifier, curriculum, toolchain) once, in one place, behind a clean HTTP boundary - and connect any RL trainer to it with a few lines of configuration.
...
- **Live environment:** [huggingface.co/spaces/imvetri/arm-gym](https://huggingface.co/spaces/imvetri/arm-gym-train/tree/main)
