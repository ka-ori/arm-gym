#!/usr/bin/env python3
"""ARM-Gym V11 — Qwen2.5-Coder-7B bf16+LoRA GRPO with harness-based correctness.

V11 changes over V10 (L40S 48GB — more VRAM headroom unlocks larger G and longer asm):
  - num_generations: 6 → 8         (stronger GRPO gradient; wider winner/loser spread)
  - max_completion_length: 768 → 1024 (v10 step 56 hit mean_length=604, step 59 max=768;
                                        difficulty-2 ML kernels need full room)
  - lora_rank: 24 → 32             (production setting; 33% more expressiveness)
  - lora_alpha: 48 → 64            (keeps alpha/rank=2 ratio)
  - steps (long): 200 → 250        (more training evidence; 5 checkpoints at every 50)

V10 changes retained:
  - flavor: l40sx1 (48GB)
  - attn_implementation: "sdpa"
  - temperature: 0.8, difficulty_max: 2

Memory budget (48GB L40S):
  Model 14GB + KV-cache(G=8,GQA-4heads)~1.5GB + SDPA activations~2GB + LoRA~0.5GB ≈ 18GB
  → 30GB headroom; safe.

Profiles:
  ARMGYM_PROFILE=mvp  (default): 50 steps  → ZDC-M01/arm-gym-v11-mvp-50
  ARMGYM_PROFILE=long          : 250 steps → ZDC-M01/arm-gym-v11-train-250
...
log.info("==== ARM-Gym V11 — Qwen2.5 bf16+LoRA GRPO (l40sx1, G=8, comp=1024, r=32, sdpa) ====")
...
# ── CONFIG ────────────────────────────────────────────────────────────────────
# ARMGYM_PROFILE=mvp  (default): 50 steps  → hub ZDC-M01/arm-gym-v11-mvp-50
# ARMGYM_PROFILE=long          : 250 steps → hub ZDC-M01/arm-gym-v11-train-250
@dataclass
class Cfg:
    model_id: str = "Qwen/Qwen2.5-Coder-7B-Instruct"
    hub_model_id: str = "ZDC-M01/arm-gym-v11-mvp-50"
    steps: int = 50
    # G=8 safe on L40S 48GB (GQA 4 KV heads → tiny KV cache; total ~18GB)
    ...
    # 1024 safe on L40S with sdpa; v10 step 56 mean_length=604, step 59 max=768 → needed more
...
def _apply_profile(cfg: Cfg) -> Cfg:
    p = _default_profile()
    if p == "long":
        cfg.hub_model_id = "ZDC-M01/arm-gym-v11-train-250"
        cfg.steps = 250
        cfg.out_dir = "runs/v11-250"
        # Checkpoint every 50 steps → 50/100/150/200/250; keep all 5
        cfg.save_steps = 50
...
try:
    ...
    if hf_tok := os.environ.get("HF_TOKEN"):
        try:
            ...
            _msg = (
                f"v11 GRPO LoRA: profile={os.environ.get('ARMGYM_PROFILE', 'mvp')} "
                f"steps={cfg.steps} lr={cfg.learning_rate:.2e} "
                f"temp={cfg.temperature} G={cfg.num_generations} "
                f"comp={cfg.max_completion_length} r={cfg.lora_rank} "
                f"difficulty<={cfg.difficulty_max} sdpa"
            )
            api.upload_folder(
