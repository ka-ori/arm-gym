#!/usr/bin/env python3
"""ARM-Gym V7 — Qwen2.5-Coder-7B bf16+LoRA GRPO with harness-based correctness.

V7 changes over V6 (hyperparameter tuning based on MVP-50 analysis + SuperCoder):
  - learning_rate: 5e-6 → 2e-6  (SuperCoder uses 1e-6; scaled up 2× for smaller batch)
  - beta: 0.04 → 0.01           (less KL penalty; policy was barely moving, KL max 0.0004)
  - epsilon: 0.2 → 0.25         (slightly wider clip region for small policy steps)
  - max_completion_length: 640 → 768  (reduce 22% truncation rate from MVP-50)
  - gradient_accumulation_steps: 8 → 4  (more frequent updates, zero OOM impact)
  - warmup_steps: 10 → 20       (smoother ramp for longer runs)
  - Everything else identical to V6 (pipeline, harness, rewards, model loading).
...
log.info("==== ARM-Gym V7 — Qwen2.5 bf16+LoRA GRPO (tuned hyperparams) ====")
...
# ── CONFIG ────────────────────────────────────────────────────────────────────
# ARMGYM_PROFILE=mvp  (default on main/dev): 50 steps → hub ZDC-M01/arm-gym-v7-mvp-50
# ARMGYM_PROFILE=long (200-step run):       200 steps → hub ZDC-M01/arm-gym-v7-train-200
@dataclass
class Cfg:
    model_id: str = "Qwen/Qwen2.5-Coder-7B-Instruct"
    hub_model_id: str = "ZDC-M01/arm-gym-v7-mvp-50"
    steps: int = 50
    # 4: safer on A10G 24GB at 50+ steps; 6 OOMs on same flavor
    num_generations: int = 4
    gradient_accumulation_steps: int = 4   # v7: 8→4, more frequent updates
    per_device_train_batch_size: int = 1
    lora_rank: int = 24
    lora_alpha: int = 48
    learning_rate: float = 2e-6            # v7: 5e-6→2e-6, SuperCoder-aligned
    max_prompt_length: int = 2048
    max_completion_length: int = 768       # v7: 640→768, reduce truncation
    temperature: float = 0.7
    difficulty_max: int = 1
    max_train: int = 64
    max_eval: int = 16
    warmup_steps: int = 20                 # v7: 10→20, smoother ramp
    out_dir: str = "runs/v7-mvp"
...
def _apply_profile(cfg: Cfg) -> Cfg:
    p = _default_profile()
    if p == "long":
        cfg.hub_model_id = "ZDC-M01/arm-gym-v7-train-200"
        cfg.steps = 200
        cfg.out_dir = "runs/v7-200"
        cfg.save_steps = 50
...
log.info("Config: model=%s steps=%d G=%d temp=%.1f lr=%.2e beta=0.01",
...
def build_grpo_config(cfg):
    ...
    p = dict(
        ...
        loss_type="grpo",
        beta=0.01,       # v7: 0.04→0.01, less KL penalty, policy was barely moving
        epsilon=0.25,    # v7: 0.2→0.25, wider clip region for small policy steps
...
log.info("=" * 60)
log.info("TRAINING START: %d steps | G=%d | lr=%.2e | beta=0.01 | completion=%d",
         cfg.steps, cfg.num_generations, cfg.learning_rate, cfg.max_completion_length)
log.info("=" * 60)
t0 = time.time()
try:
    trainer.train()
    ...
    if hf_tok := os.environ.get("HF_TOKEN"):
        try:
            ...
            _msg = (
                f"v7 GRPO LoRA: profile={os.environ.get('ARMGYM_PROFILE', 'mvp')} "
                f"steps={cfg.steps} lr={cfg.learning_rate:.2e} beta=0.01"
            )
            api.upload_folder(
